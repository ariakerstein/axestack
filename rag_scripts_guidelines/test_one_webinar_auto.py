#!/usr/bin/env python3
"""
Test Script - Process ONE Webinar PDF (Auto mode)
...
def main():
    print(f"\n{'='*70}")
    print(f"TEST SINGLE WEBINAR PDF (Auto Mode)")
    print(f"{'='*70}\n")
    ...
    # Insert chunks (auto mode - no confirmation)
    ...
    print(f"{'='*70}")
    print(f"✓ Processed 1 PDF: {test_pdf}")
    print(f"✓ Inserted {inserted} chunks")
    print(f"\nVerify in database:")
    print(f"  SELECT guideline_title, COUNT(*) as chunks")
    print(f"  FROM guideline_chunks")
    print(f"  WHERE guideline_title = '{file_metadata['title'][:50]}'")
    print(f"  GROUP BY guideline_title;")
    print(f"\nIf successful, run full batch:")
    print(f"  python3 process_webinar_pdfs.py")
    print(f"{'='*70}\n")
