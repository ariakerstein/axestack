-- Apply Security Fixes for Supabase Linter Warnings
-- This script fixes function_search_path_mutable and extension_in_public warnings

-- 1. Create a dedicated schema for extensions
CREATE SCHEMA IF NOT EXISTS extensions;

-- 2. Move vector extension to dedicated schema (if it exists in public)
DO $$
BEGIN
...
-- 3. Fix function search path security warnings by adding SET search_path
...
-- Update vector-related functions to use the new schema
-- Fix match_documents function to use extensions.vector
...
-- Fix insert_guideline_chunk function
...
AS $$
  ...
  VALUES (
    ...
    token_count,
    url,
    (SELECT string_to_array(trim(both '[]' from chunk_embedding_vec::text), ',')::float8[]::extensions.vector)
  )
...
-- Update search path for the current session
SET search_path TO public, extensions;
